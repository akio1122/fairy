    ...
    var input = {
      ...
      toHtml: function () {
        ...
        if (input.type !== 'select') {
          inputTag = t('<fieldset><input id="{id}" class="{clazz} swal-form-field" type="{type}" name="{name}"' +
            ' value="{value}" title="{placeholder}" placeholder="{placeholder}"></fieldset>', input)
        } else {
          inputTag = t('<fieldset><select id="{id}" class="{clazz} swal-form-field" name="{name}"' +
        ...
        if (typeof field.label !== 'undefined')
          labelTag = t("<label for='{name}'>{label}</label>", input)
