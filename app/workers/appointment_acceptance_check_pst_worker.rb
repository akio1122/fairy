class AppointmentAcceptanceCheckPstWorker
  include Sidekiq::Worker
  include Sidetiq::Schedulable

  sidekiq_options queue: 'critical', retry: false

  schedule_time_zone 'Pacific Time (US & Canada)'
  ...
  def perform(last_occurrence, current_occurrence)
    PaperTrail.whodunnit = "Jarvis Appointment Acceptance Checker (PST)"
    ...
    apts.without_hard_breaks.requires_housekeeper_confirmation.map do |apt|
      next if !City.in_pst?(apt.housekeeper.city)
